...
static void *bw_put_lock_all_winperthread (ArgStruct* a);
static void *bw_get_lock_all_winperthread (ArgStruct* a);
static void *bw_put_fence_winperthread (ArgStruct* a);
static void *bw_get_fence_winperthread (ArgStruct* a);
static void *bw_put_lock_per_rank_winperthread (ArgStruct* a);
static void *bw_get_lock_per_rank_winperthread (ArgStruct* a);
static void *bw_put_flush_winperthread (ArgStruct* a);
static void *bw_get_flush_winperthread (ArgStruct* a);
static void *bw_put_pscw_winperthread (ArgStruct* a);
static void *bw_get_pscw_winperthread (ArgStruct* a);

static rmamt_fn_t rmamt_winperthread_fns[RMAMT_OPERATIONS_MAX][RMAMT_SYNC_MAX] = {
    [RMAMT_PUT] = {
        [RMAMT_LOCK_ALL] = bw_put_lock_all_winperthread,
        [RMAMT_FENCE] = bw_put_fence_winperthread,
        [RMAMT_LOCK] = bw_put_lock_per_rank_winperthread,
        [RMAMT_FLUSH] = bw_put_flush_winperthread,
        [RMAMT_PSCW] = bw_put_pscw_winperthread,
    },
    [RMAMT_GET] = {
        [RMAMT_LOCK_ALL] = bw_get_lock_all_winperthread,
        [RMAMT_FENCE] = bw_get_fence_winperthread,
        [RMAMT_LOCK] = bw_get_lock_per_rank_winperthread,
        [RMAMT_FLUSH] = bw_get_flush_winperthread,
        [RMAMT_PSCW] = bw_get_pscw_winperthread,
    },
};

/* origin-side functions */
static void *bw_orig_lock_all (ArgStruct *a);
static void *bw_orig_lock (ArgStruct *a);
static void *bw_orig_flush (ArgStruct *a);
static void *bw_orig_fence (ArgStruct *a);
static void *bw_orig_pscw (ArgStruct *a);

static rmamt_fn_t rmamt_origin_fns[RMAMT_SYNC_MAX] = {
    [RMAMT_LOCK_ALL] = bw_orig_lock_all,
    [RMAMT_FENCE] = bw_orig_fence,
    [RMAMT_LOCK] = bw_orig_lock,
    [RMAMT_FLUSH] = bw_orig_flush,
    [RMAMT_PSCW] = bw_orig_pscw,
};
...
int main(int argc,char *argv[])
{
    ...
    if (0 == rank) {
        ...
        for (uint32_t j = min_size, step = 0 ; j <= max_size ; j <<= 1, ++step) {
            size_t sz = j * (rmamt_threads / win_count);
	    float max_time = 0.0;
            float speed = 0.0;
		...
		fprintf (output_file, "%lu,%lu,%f,%f\n", (unsigned long) j, (unsigned long) j * rmamt_threads, speed,
			 (rmamt_threads * rmamt_iterations * 1000000000.0) / max_time);
	    }
...
#define DEFINE_ORIGIN_THREAD_FN(sync, type, fn, init_fn, start_sync, end_sync, fini_fn) \
    static void *bw_ ## type ## _ ## sync ## _winperthread (ArgStruct* a) { \
	...
	if (rmamt_bind_threads) {					\
        ...
        for (uint32_t j = min_size, cycle = 0 ; j <= max_size ; j <<= 1) { \
            ...
            for (int l = 0 ; l < RMAMT_WARMUP_ITERATIONS ; l++) {       \
                fn (obuf + tid * j, j, MPI_BYTE, 1, 0, j, MPI_BYTE,     \
                    a->win);                                            \
            }                                                           \
                                                                        \
            end_sync;                                                   \
...
#define DEFINE_ORIGIN_FN(sync, init_fn, start_sync, end_sync, fini_fn)  \
    static void *bw_orig_ ## sync (ArgStruct *a) {                      \
        size_t max_size = a->max_size;                                  \
        size_t min_size = a->min_size;                                  \
                                                                        \
        init_fn;                                                        \
